...
/* 管理LCD重要参数 */
static tLcdDevice sLcdDevice;  //默认为竖屏
...
static void LCD_WriteRAM_Prepare(void)
{
 	LCD->LCD_REG=sLcdDevice.wramcmd;	  
}
...
{	 
	if(sLcdDevice.dir==1)
		Xpos=sLcdDevice.width-1-Xpos;			//横屏其实就是调转x,y坐标
	LCD_WriteReg(sLcdDevice.setxcmd, Xpos);
	LCD_WriteReg(sLcdDevice.setycmd, Ypos); 
}
...
{
    if(sLcdDevice.dir==1)
        x=sLcdDevice.width-1-x;//横屏其实就是调转x,y坐标
    LCD_WriteReg(sLcdDevice.setxcmd,x);
    LCD_WriteReg(sLcdDevice.setycmd,y);
...
void LCD_Display_Dir(uint8_t dir)
{
	...
	{
		sLcdDevice.dir      = 0;
		sLcdDevice.width    = 240;
		sLcdDevice.height   = 320;
		
        sLcdDevice.wramcmd  = R34;
        sLcdDevice.setxcmd  = R32;
        sLcdDevice.setycmd  = R33;  

	}
    else
	{	  
		sLcdDevice.dir      = 1;
		sLcdDevice.width    = 320;
		sLcdDevice.height   = 240;
		
        sLcdDevice.wramcmd  = R34;
        sLcdDevice.setxcmd  = R33;
        sLcdDevice.setycmd  = R32;  
	} 
...
{ 										  
	...
	delay_ms(50); // delay 50 ms 
  	sLcdDevice.id = LCD_ReadReg(0x0000);   
  		 
 	if(sLcdDevice.id==0x9325)//9325
	{
...
{
	u32 index=0;
	u32 totalpoint = sLcdDevice.width*sLcdDevice.height; 	//得到总点数
	...
	if(!mode) //非叠加方式
	{
	    ...
	    {   
			...
			{			    
				...
				temp<<=1;
				y++;
				if(x>=sLcdDevice.width){POINT_COLOR=colortemp;return;}//超区域了
				if((y-y0)==size)
				{
					y=y0;
					x++;
	...
	}else//叠加方式
	{
	    ...
	    {   
			...
			{			    
				...
				temp<<=1;
				y++;
				if(x>=sLcdDevice.height){POINT_COLOR=colortemp;return;}//超区域了
				if((y-y0)==size)
				{
					y=y0;
					x++;
...
void LCD_ShowStringNotStack(u16 x,u16 y,u16 width,u16 height,u8 size,u8 *p)
{         
	u8 x0=x;
	width+=x;
	height+=y;
...
{ 
    ...
    SPI_Flash_Read(pBuffer,FLASH_FONT_ADDRESS+pos,32);	/* 读取外部Flash中的字库 */
}
...
void BSP_LCD_DrawChar_CH ( uint16_t startx, uint16_t starty, uint16_t usChar )
{
	...
	if ( startx>=sLcdDevice.width || starty>=sLcdDevice.height )
		return ;
